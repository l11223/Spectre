...
object ModuleBackupUtils {
    ...
    private const val MODULE_DIR = "/data/adb/.sp_ext"
    ...
    suspend fun autoBackupModule(context: Context, file: File, originalFileName: String?, subDir: String): String? {
        return withContext(Dispatchers.IO) {
            ...
            val webdavJob = async {
                if (me.yuki.spectre.ui.theme.BackupConfig.isBackupEnabled) {
                     try {
                         val basePath = me.yuki.spectre.ui.theme.BackupConfig.webdavPath
                         ...
                         val webDavResult = WebDavUtils.uploadFile(
                            me.yuki.spectre.ui.theme.BackupConfig.webdavUrl,
                            me.yuki.spectre.ui.theme.BackupConfig.webdavUsername,
                            me.yuki.spectre.ui.theme.BackupConfig.webdavPassword,
                            file,
                            cleanSubDir,
                            originalFileName
                         )
            ...
            val localJob = async {
                if (APApplication.sharedPreferences.getBoolean("auto_backup_module", false)) {
                    try {
                        val baseBackupDir = File(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS), "Spectre/ModuleBackups")
