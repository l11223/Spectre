...
suspend fun getBugreportFile(context: Context): File = withContext(Dispatchers.IO) {
    ...
    shell.newJob().add("tar -czf ${bootlogFile.absolutePath} -C /data/adb/.fk/tmp .").exec()
    ...
    shell.newJob().add("cp /data/adb/.fk/package_config ${packageConfigFile.absolutePath}").exec()
    ...
    PrintWriter(FileWriter(buildInfo)).use { pw ->
        ...
        pw.println("Core: ${Version.installedKPVString()}")
        pw.println("Spectre: ${Version.installedApdVString}")
        val safeMode = false
    ...
    val targetFile = File(context.cacheDir, "Spectre_bugreport_${current}.tar.gz")
