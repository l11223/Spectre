...
import me.yuki.spectre.ui.CrashHandleActivity
import me.yuki.spectre.ui.component.KpmAutoLoadManager
import me.yuki.spectre.util.APatchCli
import me.yuki.spectre.util.APatchKeyHelper
import me.yuki.spectre.ui.theme.MusicConfig
import me.yuki.spectre.util.MusicManager
import me.yuki.spectre.util.Version
import me.yuki.spectre.util.getRootShell
import me.yuki.spectre.util.rootShellForResult
import me.yuki.spectre.util.verifyAppSignature
...
// STEALTH (DET-04): Generic log tag to avoid detection in logcat
...
class APApplication : Application(), Thread.UncaughtExceptionHandler, ImageLoaderFactory {
    ...
    companion object {
        // STEALTH (DET-01): Paths synchronized with daemon-side defs.rs
        // Old paths (/data/adb/apd, /data/adb/ap/) are well-known detection targets
        const val APD_PATH = "/data/adb/.core"

        @Deprecated("No more KPatch ELF from 0.11.0-dev")
        const val KPATCH_PATH = "/data/adb/.kp"
        const val SUPERCMD = "/system/bin/truncate"
        const val APATCH_FOLDER = "/data/adb/.fk/"
        private const val APATCH_BIN_FOLDER = APATCH_FOLDER + "lib/"
        private const val APATCH_LOG_FOLDER = APATCH_FOLDER + "tmp/"
        private const val APD_LINK_PATH = APATCH_BIN_FOLDER + ".core"
        ...
        const val GLOBAL_NAMESPACE_FILE = "/data/adb/.ns"
        const val MAGIC_MOUNT_FILE = "/data/adb/.mm"
    ...
    override fun onCreate() {
        ...
        me.yuki.spectre.util.LauncherIconUtils.applySaved(this)
                ...
                .addInterceptor { block ->
                    block.proceed(
                        block.request().newBuilder()
                            // STEALTH (DET-05): Generic User-Agent to avoid network fingerprinting
                            .header("User-Agent", "Android/${BuildConfig.VERSION_CODE}")
        ...
        me.yuki.spectre.ui.theme.SoundEffectConfig.load(this)
