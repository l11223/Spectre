...
#
# boot_patch.sh      script        A script to patch boot image for Spectre.
...
# bootimg         executable    Magisk tool to unpack boot.img.
#
#######################################################################################
...
command -v ./bootimg >/dev/null 2>&1 || { >&2 echo "- Command bootimg not found!"; exit 1; }
command -v ./kptools >/dev/null 2>&1 || { >&2 echo "- Command kptools not found!"; exit 1; }

if [ ! -f kernel ]; then
echo "- Unpacking boot image"
./bootimg unpack "$BOOTIMAGE" >/dev/null 2>&1
  if [ $? -ne 0 ]; then
    >&2 echo "- Unpack error: $?"
    exit $?
  fi
...
if [ ! $(./kptools -i kernel -f | grep CONFIG_KALLSYMS=y) ]; then
	...
	echo "- Spectre requires CONFIG_KALLSYMS to be Enabled."
