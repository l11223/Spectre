...
pub fn on_post_data_fs(superkey: Option<String>) -> Result<()> {
    // SECURITY (VULN-11): umask(0) creates world-readable/writable files.
    // This exposed package_config, logs, and temp files to any app.
    utils::umask(0o077);
    ...
    // Power optimization: Only collect boot logs if debug mode is enabled.
    // Saves ~2 processes and 120s of CPU time per boot.
    ...
    if debug_mode {
        ...
        args = vec![
            "-s", "9", "30s", "logcat", "-b", "main,system,crash",
            "-f", &logcat_path, "logcatcher-bootlog:S", "&",
        ];
        let _ = unsafe {
            Command::new("timeout")
                .process_group(0)
                .pre_exec(|| { switch_cgroups(); Ok(()) })
                .args(args)
                .spawn()
        };
        args = vec!["-s", "9", "30s", "dmesg", "-w"];
        let _result = unsafe {
            Command::new("timeout")
                .process_group(0)
                .pre_exec(|| { switch_cgroups(); Ok(()) })
        ...
        info!("Boot logging disabled (create debug_mode file to enable).");
    }

    // Stealth: Removed KERNELPATCH_VERSION and KERNEL_VERSION env var logging
    // These strings are detectable in logcat output
    ...
    // SECURITY (VULN-05): Pass empty string instead of SuperKey to Lua modules.
    // Previously, the SuperKey was leaked to ALL installed Lua modules.
    if let Err(e) = module::exec_stage_lua("post-fs-data", true, "")
    {
...
fn run_stage(stage: &str, superkey: Option<String>, block: bool) {
    // SECURITY (VULN-11): Restrict file permissions
    utils::umask(0o077);
    ...
    // SECURITY (VULN-05): Never pass SuperKey to Lua module code
    if let Err(e) = module::exec_stage_lua(stage, block, "") {
...
fn run_uid_monitor() {
    ...
    let mut command = &mut Command::new("/data/adb/.core");
    {
...
pub fn start_uid_listener() -> Result<()> {
    ...
    // Power optimization: Increased debounce from 1s to 5s.
    // App installs trigger rapid consecutive changes to packages.list.
    // With 1s debounce, we still process too many redundant refreshes.
    // 5s covers typical batch install scenarios.
