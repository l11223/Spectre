...
pub fn privilege_apd_profile(superkey: &Option<String>) {
    ...
    // Stealth: Use a less suspicious SELinux context
    // "u:r:magisk:s0" is an instant detection flag for anti-cheat
...
pub fn init_load_su_path(superkey: &Option<String>) {
    let su_path_file = "/data/adb/.fk/su_path";
        ...
        Some(superkey_cstr) => {
            unsafe {
                ...
                if pid < 0 {
                    ...
                    // STEALTH: ALL detectable env vars removed.
                    // KERNELPATCH, KERNELPATCH_VERSION, KERNEL_VERSION, KP_READY
                    // were all readable via /proc/<pid>/environ by any app.
