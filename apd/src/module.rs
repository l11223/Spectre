...
// Power optimization: Cache module paths to avoid repeated directory scans
// during boot (foreach_module is called 5-10 times in boot sequence)
static MODULE_CACHE: Mutex<Option<Vec<PathBuf>>> = Mutex::new(None);
...
pub fn foreach_module(
    ...
    // Power optimization: Use cached module list for Active modules
    // to avoid repeated fs::read_dir() calls during boot sequence
    if module_type == ModuleType::Active {
    ...
    // Non-cached path for Updated modules
...
/// Invalidate the module cache (call after module install/uninstall)
...
pub fn exec_script<T: AsRef<Path>>(path: T, wait: bool) -> Result<()> {
    ...
    command = command
        ...
        .arg("sh")
        .arg(path.as_ref())
        // STEALTH (DET-03): Removed detectable env vars
...
pub fn install_module(zip: &str) -> Result<()> {
    ...
    invalidate_module_cache(); // Clear cache after install
    result
}
...
pub fn uninstall_module(id: &str) -> Result<()> {
    ...
    invalidate_module_cache(); // Clear cache after uninstall
    mark_update()?;
    Ok(())
}
...
pub fn save_text<P: AsRef<Path>>(filename: P, content: &str) -> std::io::Result<()> {
    let config_dir = defs::CONFIG_DIR;
    ...
    if !canonical.starts_with(config_dir) {
        return Err(std::io::Error::new(
            ...
            format!("path traversal rejected: {:?}", filename.as_ref()),
        ));
    }
...
pub fn load_text<P: AsRef<Path>>(filename: P) -> std::io::Result<String> {
    ...
    let _ = ensure_dir_exists(config_dir);
    let path = Path::new(config_dir).join(&filename);
    // SECURITY (VULN-10): Validate path to prevent directory traversal reads
    let canonical = path.canonicalize().unwrap_or(path.clone());
    if !canonical.starts_with(config_dir) {
        return Err(std::io::Error::new(
            std::io::ErrorKind::PermissionDenied,
...
pub fn run_lua(id: &str, function: &str, on_each_module: bool, _wait: bool) -> mlua::Result<()> {
    // SECURITY (VULN-04): Use safe Lua runtime. unsafe_new() allows loading
    // arbitrary native .so files from module directories via require().
    let lua = Lua::new();
...
pub fn run_action(id: &str) -> Result<()> {
    let action_script_path = format!("{}{}/action.sh", defs::MODULE_DIR, id);
