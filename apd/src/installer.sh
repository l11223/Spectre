...
boot_actions() {
  if [ ! -f "$NVBASE/jq" ]; then
    local apk_path=$(find /data/app -name "base.apk" -path "*/me.yuki.spectre-*" 2>/dev/null | head -n 1)
...
install_module() {
  ...
  local MODDIRNAME=.sp_ext
  $BOOTMODE && MODDIRNAME=.sp_ext_up
  ...
  if $BOOTMODE; then
    mktouch $NVBASE/.sp_ext/$MODID/update
    rm -rf $NVBASE/.sp_ext/$MODID/remove 2>/dev/null
    rm -rf $NVBASE/.sp_ext/$MODID/disable 2>/dev/null
    cp -af $MODPATH/module.prop $NVBASE/.sp_ext/$MODID/module.prop
  fi
