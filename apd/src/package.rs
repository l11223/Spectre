...
pub fn read_ap_package_config() -> Vec<PackageConfig> {
    ...
    for _ in 0..max_retry {
        let file = match File::open("/data/adb/.fk/package_config") {
            Ok(file) => file,
            Err(e) => {
...
pub fn write_ap_package_config(package_configs: &[PackageConfig]) -> io::Result<()> {
    let max_retry = 5;
    for _ in 0..max_retry {
        let temp_path = "/data/adb/.fk/package_config.tmp";
        ...
        if let Err(e) = std::fs::rename(temp_path, "/data/adb/.fk/package_config") {
